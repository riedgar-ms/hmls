...
This package trains neural network models (any architecture registered via the `hmls.models` entry-point group) by having two models play against each other on randomly generated maps. It uses the REINFORCE algorithm (policy gradient with baseline normalisation) to improve each model's policy.
