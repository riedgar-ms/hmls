...
def test_missed_fire_penalty_enemy_ahead() -> None:
    """Non-fire action when enemy is directly ahead incurs missed_fire_penalty."""
    reward_fn = DefaultReward()
    entry = _make_entry(action=Action.MOVE_FORWARD)
    ...
    # step_penalty + missed_fire_penalty + enemy_in_cone_reward (enemy is also in cone)
    expected = -0.01 + -0.1 + 0.01
    assert abs(reward - expected) < 1e-7


def test_missed_fire_penalty_not_applied_on_fire() -> None:
    """Missed fire penalty is NOT applied when the action was FIRE (hit)."""
    reward_fn = DefaultReward()
    entry = _make_entry(action=Action.FIRE, hit=True)
    patch = _make_patch_with_enemy_ahead()
    reward = reward_fn.compute_step_reward(
    ...
    # step_penalty + hit_reward + enemy_in_cone_reward (enemy visible)
    expected = -0.01 + 0.5 + 0.01
    assert abs(reward - expected) < 1e-7


def test_missed_fire_penalty_not_applied_no_enemy() -> None:
    """No missed fire penalty when there is no enemy directly ahead."""
    reward_fn = DefaultReward()
...
def test_pass_penalty_deliberate() -> None:
    """A deliberate PASS incurs the pass_penalty."""
    reward_fn = DefaultReward()
    ...
    # step_penalty + pass_penalty
    expected = -0.01 + -0.02
    assert abs(reward - expected) < 1e-7


def test_pass_penalty_not_applied_on_invalid() -> None:
    """Invalid action converted to PASS does NOT incur pass_penalty."""
    reward_fn = DefaultReward()
    ...
    # step_penalty + invalid_move_penalty (no pass_penalty)
    expected = -0.01 + -0.1
    assert abs(reward - expected) < 1e-7
...
def test_enemy_in_cone_reward_single_enemy() -> None:
    ...
    # step_penalty + enemy_in_cone_reward * 1
    expected = -0.01 + 0.01
    assert abs(reward - expected) < 1e-7
...
def test_enemy_in_cone_reward_multiple_enemies() -> None:
    ...
    # step_penalty + enemy_in_cone_reward * 2
...
def test_enemy_in_cone_reward_not_applied_when_fogged() -> None:
    ...
    # Just step_penalty (no cone reward since enemy is fogged)
    assert abs(reward - (-0.01)) < 1e-7
...
def test_enemy_in_cone_friendly_not_counted() -> None:
    ...
    # Just step_penalty (friendly not counted)
    assert abs(reward - (-0.01)) < 1e-7


def test_missed_fire_friendly_ahead_no_penalty() -> None:
    """No missed fire penalty when a friendly tank is directly ahead."""
    reward_fn = DefaultReward()
    entry = _make_entry(action=Action.MOVE_FORWARD)
    patch = _make_empty_patch()
    half = 9 // 2
    ...
    # Just step_penalty (no missed_fire_penalty for friendly)
    assert abs(reward - (-0.01)) < 1e-7
