...
def test_consecutive_pass_reward_escalates() -> None:
    ...
    r1 = reward_fn.compute_step_reward(_make_entry(action=Action.PASS), patch=patch, team="alpha")
    assert abs(r1 - (-0.02)) < 1e-7

    # Second consecutive pass: streak=2, penalty = -0.02 × 2 = -0.04
    r2 = reward_fn.compute_step_reward(_make_entry(action=Action.PASS), patch=patch, team="alpha")
    assert abs(r2 - (-0.04)) < 1e-7

    # Third consecutive pass: streak=3, penalty = -0.02 × 3 = -0.06
    r3 = reward_fn.compute_step_reward(_make_entry(action=Action.PASS), patch=patch, team="alpha")
    assert abs(r3 - (-0.06)) < 1e-7


def test_consecutive_pass_reward_resets_on_valid_move_forward() -> None:
    ...
    r = reward_fn.compute_step_reward(_make_entry(action=Action.PASS), patch=patch, team="alpha")
    assert abs(r - (-0.02)) < 1e-7


def test_consecutive_pass_reward_resets_on_fire_hit() -> None:
...
def test_consecutive_pass_reward_not_reset_by_fire_miss() -> None:
...
def test_consecutive_pass_reward_not_reset_by_turn() -> None:
...
def test_consecutive_pass_reward_resets_on_episode() -> None:
